...
class Admins(Base):
    ...
    date_of_add = mapped_column(DateTime, nullable=False, default=datetime.now())
    date_of_update = mapped_column(DateTime)

    selection: Mapped[List['Selection']] = relationship('Selection', uselist=True, back_populates='admin')
    users: Mapped[List['Users']] = relationship('Users', uselist=True, back_populates='admin')


class Selection(Base):
    __tablename__ = 'selection'
    __table_args__ = (
        ForeignKeyConstraint(['admin_id'], ['admins.tg_id'], ondelete='CASCADE', name='selection_admin_id_fkey'),
        PrimaryKeyConstraint('id', name='selection_pkey')
    )

    id = mapped_column(BigInteger, server_default=text("nextval('selection_id_seq'::regclass)"))
    ...
    admin: Mapped['Admins'] = relationship('Admins', back_populates='selection')
...
class Users(Base):
    ...
    id = mapped_column(BigInteger, nullable=False, server_default=text("nextval('users_id_seq'::regclass)"))
    name = mapped_column(String, nullable=False)
...
class BuyRequest(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['user_id'], ['users.tg_id'], ondelete='CASCADE', name='buy_request_user_id_fkey'),
        PrimaryKeyConstraint('id', name='buy_request_pkey')
    )

    id = mapped_column(BigInteger, server_default=text("nextval('buyrequest_id_seq'::regclass)"))
...
class SellRequest(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['user_id'], ['users.tg_id'], ondelete='CASCADE', name='sell_request_user_id_fkey'),
        PrimaryKeyConstraint('id', name='sell_request_pkey')
    )

    id = mapped_column(BigInteger, server_default=text("nextval('sellrequest_id_seq'::regclass)"))
    ...
    user: Mapped['Users'] = relationship('Users', back_populates='sell_request')


class UsersConlultation(Base):
    __tablename__ = 'users_conlultation'
    __table_args__ = (
        ForeignKeyConstraint(['user_id'], ['users.tg_id'], ondelete='CASCADE', name='users_conlultation_user_id_fkey'),
        PrimaryKeyConstraint('id', name='users_conlultation_pkey')
    )

    id = mapped_column(BigInteger, server_default=text("nextval('usersconsultation_id_seq'::regclass)"))
    date_of_add = mapped_column(DateTime, nullable=False, default=datetime.now())
    user_id = mapped_column(BigInteger, nullable=False)

    user: Mapped['Users'] = relationship('Users', back_populates='users_conlultation')


class UsersMortgage(Base):
    __tablename__ = 'users_mortgage'
    __table_args__ = (
        ForeignKeyConstraint(['user_id'], ['users.tg_id'], ondelete='CASCADE', name='users_mortgage_user_id_fkey'),
        PrimaryKeyConstraint('id', name='users_mortgage_pkey')
    )

    id = mapped_column(BigInteger, server_default=text("nextval('usersmortgage_id_seq'::regclass)"))
    ...
    user: Mapped['Users'] = relationship('Users', back_populates='users_mortgage')


class UsersQuestions(Base):
    __tablename__ = 'users_questions'
    __table_args__ = (
        ForeignKeyConstraint(['user_id'], ['users.tg_id'], ondelete='CASCADE', name='users_questions_user_id_fkey'),
        PrimaryKeyConstraint('id', name='users_questions_pkey')
    )

    id = mapped_column(BigInteger, server_default=text("nextval('usersquestions_id_seq'::regclass)"))
    question = mapped_column(Boolean, nullable=False)
    date_of_add = mapped_column(DateTime, nullable=False, default=datetime.now())
    user_id = mapped_column(BigInteger, nullable=False)

    user: Mapped['Users'] = relationship('Users', back_populates='users_questions')
