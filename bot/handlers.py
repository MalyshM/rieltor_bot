...
from selection_crud import (
    create_selection,
    get_selection,
    update_selection,
    get_all_selections,
    get_all_selections_by_admin,
)
...
async def start_handler(msg: Message, command: CommandObject):
    ...
    if args:
        try:
            ...
            try:
                await msg.message.answer(
                    "Выберите интересующий вас раздел", reply_markup=kb.menu
                )
            except:
                await msg.answer(
                    "Выберите интересующий вас раздел", reply_markup=kb.menu
        ...
        await msg.answer(
            ...
            reply_markup=kb.type_of_user_kb,
        )
        ...
        if isinstance(int(msg.text), int):
            ...
            async with connect_db() as session:
                res = await update_user(
                    session, msg.from_user.id, admin_id=int(msg.text)
                )
                print(res.tg_id)
                print(res.admin_id)
    except:
...
async def another_rieltor_code(message: Message, state: FSMContext):
    try:
        ...
        if isinstance(int(message.text), int):
            ...
            async with connect_db() as session:
                res = await update_user(
                    session, message.from_user.id, admin_id=int(message.text)
                )
                print(res.tg_id)
                print(res.admin_id)
    except:
    ...
    await message.message.answer(
        "Введите Ваш новый код риелтора:", reply_markup=kb.exit_to_menu_kb
    )
...
async def admin(msg: Message, state: FSMContext):
    ...
    try:
        async with connect_db() as session:
            ...
            if resp is None:
                await msg.message.answer(
                    "Вам запрещен вход в этот отдел", reply_markup=kb.menu
                )
            if resp.email == "asd":
                await state.set_state(UserData.email)
                await msg.message.answer(
                    "Введите свою почту для дальнейшего получения заявок от пользователей"
                )
            else:
                try:
                    await msg.message.answer(
                        "Выберите интересующий вас раздел", reply_markup=kb.admin_kb
                    )
                except:
                    await msg.answer(
                        "Выберите интересующий вас раздел", reply_markup=kb.admin_kb
...
async def object_deals(callback_query: CallbackQuery, state: FSMContext):
    await callback_query.message.answer(
        "Выберите интересующую вас подборку", reply_markup=kb.type_of_object_deals_kb
    )
...
async def add_deal_link(msg: Message, state: FSMContext):
    ...
    data = {
        "type_of": admin_data["type_of"],
        "admin_id": admin_data["tg_id"],
        "link": msg.text,
    }
    try:
        async with connect_db() as session:
...
async def menu(msg: Message, state: FSMContext):
    ...
    if user.phone_number is None:
        ...
        res = send_email(
            ...
            subject="Пришла заявка на консультацию по телефону",
...
async def menu(msg: Message, state: FSMContext):
    ...
    res = send_email(
        to_email=admin.email,
        subject="Пришел вопрос от пользователя",
        message=f"Данные пользователя: {prettify_dict_str(user_data)},\nВопрос - {msg.text}",
    )
...
async def deals_of_the_week(callback_query: CallbackQuery, state: FSMContext):
    ...
    if data[1] != "Новостройка":
        async with connect_db() as session:
            ...
            await callback_query.message.answer(
                f"{res.link if not isinstance(res,str) else res}",
                reply_markup=kb.exit_to_menu_kb,
            )
    else:
        await callback_query.message.answer(
...
async def deals_of_the_week(callback_query: CallbackQuery, state: FSMContext):
    ...
    async with connect_db() as session:
        ...
        user = await get_user(session, user_data["id"])
        if data[1] == "Акц":
            data[1] = "Акции и траншевая ипотека"
        res = await get_selection(session, user.admin_id, data[1])
        await callback_query.message.answer(
            f"{res.link if not isinstance(res,str) else res}",
            reply_markup=kb.exit_to_menu_kb,
...
async def sell_real_estate_true(callback_query: CallbackQuery, state: FSMContext):
    ...
    print(data)
    await callback_query.message.answer(
        f"Выберите тип объекта: ", reply_markup=kb.type_of_object
    )
...
async def get_birth_date(message: types.Message, state: FSMContext):
    ...
    if user_data["IS_TRUE"] != "True":
        ...
        await message.answer(
            "Введите площадь объекта в квадратных метрах.\nПример: 50",
            reply_markup=kb.exit_to_menu_kb,
        )
...
async def type_of_object_final(message: types.Message, state: FSMContext):
    ...
    res = send_email(
        to_email=admin.email,
        subject="Пришла заявка на продажу",
        message=f"Данные пользователя: {prettify_dict_str(user_data)}",
    )
    print(res)
    await message.answer(
