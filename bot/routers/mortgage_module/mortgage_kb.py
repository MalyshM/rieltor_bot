...
mortgage_info = InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(
        text="Получить информацию по ипотечным программам", callback_data="years")],
    [InlineKeyboardButton(text="Подать заявку на ипотеку",
                          callback_data="make_mortgage")],
    [InlineKeyboardButton(text="Назад в меню", callback_data="back_to_menu")],
])
number_of_children_kb = InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(
        text="2 и более", callback_data="18_or_older,2 и более")],
    [InlineKeyboardButton(
        text="1 ребенок", callback_data="is_2018_+,1 ребенок")],
    [InlineKeyboardButton(
        text="Нет", callback_data="make_mortgage,0,1,1,1,1")],
    [InlineKeyboardButton(text="Назад в меню", callback_data="back_to_menu")],
])
...
is_2018_ = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(
        text="Нет", callback_data="make_mortgage,1,1,0,0,1")],
    [InlineKeyboardButton(text="Назад в меню", callback_data="back_to_menu")],
])
...
async def get_year_calendar(offset: int = 0) -> InlineKeyboardMarkup:
    ...
    for i in range(15):
        builder.button(text=f"{start_year}",
                       callback_data=f"{start_year},number_of_children")
        start_year += 1
    builder.button(text=f"Назад", callback_data=f"{offset},year_of_birth,back")
    builder.button(text=f"В меню", callback_data=f"menu")
    builder.button(
        text=f"Вперед", callback_data=f"{offset},year_of_birth,forward")
    builder.adjust(3)
